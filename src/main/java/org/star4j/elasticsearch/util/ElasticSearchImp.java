...
public class ElasticSearchImp {
	...
	static {
		try {
			...
			InputStream in = ElasticSearchImp.class.getResourceAsStream("/elasticsearch.properties");
			props.load(in);// 加载文件
			// 读取信息
			cluster_name = props.getProperty("cluster_name");
			cluster_serverip = props.getProperty("cluster_serverip");
			cluster_serverport = props.getProperty("cluster_serverport");
	...
	public static TransportClient getClient() {
		...
		try {
			client.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName(cluster_serverip), Integer.parseInt(cluster_serverport)));// 增加地址和端口
	 ...
	 * 
	 * @param entity 实体
	 * @param methodNameParm 实体中待转换成json的字段
	 * @return 返回json
	 * @throws Exception
	 */
		...
		try {
			for (String methodName : methodNameParm) {
				...
				if (value instanceof String){
					...
					fieldValue = new SimpleDateFormat("yyyy-MM-dd hh:mm").format((Date) value);
				}else {
				...
				String fieldName = StringUtil.firstToLower(methodName.replace("get", ""));// 去掉“get”，并将首字母小写
	...
	public static boolean addMapDocToIndex(String type, String docId, Map<String, String> mapParam) {
		...
		String strResult = "新增文档成功：index：" + index + " |  type: " + gettype + " | id: " + id + " | version: " + version + " | status: " + status.getStatus();
	...
	public static boolean updateDoc(String type, String docId, Object entity) {
		...
		updateRequest.id(docId);
		try {
			updateRequest.doc(createEntityJson(entity));
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
